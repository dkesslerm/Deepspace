...
class TestP2
	def self.main
		...
		3.times do
			...
			numeric_damage_test = NumericDamage.new(nWeapons, nShields)
			weapons = []
			4.times do
			...
			type_damage_test = SpecificDamage.new(weapons, nShields)
			...
			copy_test = numeric_damage_test.copy
			puts "Copy Damage: \n\t#{copy_test}"
			puts ""
            ...
            wp = []
            wp << Weapon.new("wp1", WeaponType::LASER, 2)
            wp << Weapon.new("wp2", WeaponType::MISSILE, 3)
            sp = []
            sp << "test"
            sp << "does nothing"
			srink_damage_test = type_damage_test.adjust(wp,sp)
			puts "\tAdjusted Damage: #{srink_damage_test.to_s}"
			puts "\tAdjusted Damage Weapons: #{srink_damage_test.weapons.to_s}"
			puts "" 
		...
		enemy_test = EnemyStarShip.new("enemy tank ship", 1.123, 2.123124, Loot.new(1,2,3,4,5), NumericDamage.new(3,3))
		...
		universe_test.init(["prueba"])
		puts "Game Universe: #{universe_test}"
		puts ""

		puts "Testing getters:"
		puts "\tGame State: #{universe_test.state}"
		puts ""
